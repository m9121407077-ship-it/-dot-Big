...
def dl(s):
 ...
 for a in range(1,5):
  try:
   ...
   with requests.get(s['url'],stream=True,timeout=(30,240),allow_redirects=True,headers={'User-Agent':'Qarain-Atlas-Divar/1.0'}) as r:
...
rows=con.execute('SELECT COUNT(*) FROM raw').fetchone()[0]; desc=con.execute('DESCRIBE raw').fetchall(); cols=[r[0] for r in desc]; types={r[0]:r[1] for r in desc}
print('ROWS',rows,'COLS',len(cols),flush=True)
...
con.execute('CREATE VIEW clean AS SELECT *, '+norm('title')+' title_norm, '+norm('description')+''' description_norm,
 ...
 FROM raw''')
# Non-destructive: every source row and column is preserved; only helper columns added.
...
null_expr=[f'SUM(CASE WHEN "{c}" IS NULL THEN 1 ELSE 0 END)' for c in cols]; nr=con.execute('SELECT '+','.join(null_expr)+' FROM raw').fetchone(); nulls={c:{'null_count':int(nr[i]),'null_pct':round(100*nr[i]/rows,4)} for i,c in enumerate(cols)}
coverage={
 'row_count':rows,'source_column_count':len(cols),'columns':cols,'types':types,
...
# Missingness by category for every source column.
parts=[]
for c in cols:
 cc=c.replace("'","''"); parts.append(f'''SELECT cat2_slug,'{cc}' column_name,COUNT(*) rows,SUM(CASE WHEN "{c}" IS NULL THEN 1 ELSE 0 END) null_count,ROUND(100.0*SUM(CASE WHEN "{c}" IS NULL THEN 1 ELSE 0 END)/COUNT(*),4) null_pct FROM raw GROUP BY cat2_slug''')
con.execute(f"COPY ({' UNION ALL '.join(parts)}) TO '{escpath(REP/'missingness_by_cat2.parquet')}' (FORMAT PARQUET,COMPRESSION ZSTD)")

# Asking-price context. Currency unit deliberately NOT assumed because dataset README does not specify it.
valid="cat2_slug='residential-sell' AND price_value>0 AND building_size BETWEEN 10 AND 1000"
...
# Deterministic listing-language claims: claim != fact.
...
# Duplicate candidates on observable fields only; dataset has no stable ad id.
...
(REP/'README_FA.md').write_text(f'''# استخراج رسمی املاک دیوار برای اطلس قرائن\n\nمنبع `divarofficial/real_estate_ads`، مجوز ODbL. تمام **{rows:,}** ردیف خوانده شد. ستون‌های منبع: **{len(cols)}**. شهرها: **{coverage['cities']}**. بازه واقعی created_at_month: **{coverage['date_min']} تا {coverage['date_max']}**.\n\nقفل: `price_value` قیمت آگهی است نه معامله؛ واحد پول در README صریح نیست و در خروجی `raw_unit` مانده است. در clean_full هیچ ردیفی حذف نشده و همه ستون‌های منبع حفظ شده‌اند.\n''',encoding='utf-8')
print('FINAL',json.dumps({'rows':rows,'cols':len(cols),'cities':coverage['cities'],'date_min':coverage['date_min'],'date_max':coverage['date_max'],'checks':checks},ensure_ascii=False,default=str),flush=True)
